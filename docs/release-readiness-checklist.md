...
| ⚠️ | **Per-bullet citation enforcement** | Validator currently checks at section level — at least one citation per section. A section with mixed cited/uncited bullets passes. Documented as Known Limitation. |
| ⚠️ | **Untagged Java fence detection** | Only ` ```java ` tagged blocks are caught. Untagged ` ``` ` blocks containing Java keywords slip through. Documented as Known Limitation. |
| ⚠️ | **Placeholder text (`N/A`, `TBD`) satisfying "none found"** | Empty-section check only catches whitespace; literal placeholder strings pass. Documented as Known Limitation. |
...
| ❌ | **Stage 3 prompts include config key-VALUES (not just key prefixes)** | Codex High #4 — biggest quality regression. `generate.py:_collect_source_blob` never reads config files; `crawler._extract_yaml_prefixes` keeps prefixes only. Blocks enterprise rollout because config-driven behavior is invisible to generated skills. |
...
| | Item | Notes |
|---|---|---|
| ✅ | **Unit tests cover validator, crawler, generate helpers** | 71 tests; runs in <13s. `python3 -m unittest discover -v`. |
...
- **Other engineers in the same org, with light hand-holding:** ✅ ready once section 1's ❌ rows are addressed (Stage-3 config values, god-class decision, basename ambiguity).
