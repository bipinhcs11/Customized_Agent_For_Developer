...
```mermaid
flowchart LR
    A[Developer in<br/>VSCode / IntelliJ] -->|opens| B[Claude Code /<br/>Copilot Chat /<br/>Cowork panel]
...
sequenceDiagram
    ...
    participant IDE as IDE + AI panel<br/>(Claude Code / Copilot Chat / Cowork)
    ...
    Dev->>IDE: Open IDE with an AI assistant panel attached
    end
...
1. **Setup (once).** Clone this agent repo so the host agent — Claude Code, GitHub Copilot Chat, Codex, or Claude Cowork — can discover `skills/skill-generator/SKILL.md`.
...
3. **Pre-flight.** The skill asks for the target repo's absolute path, confirms Python 3.10+, and asks whether to include tests.
4. **Pipeline.** The skill drives the Python CLI through Crawl -> Plan -> Generate -> Link. Stage 1 is deterministic (no LLM). Stages 2-4 each follow the same emit/ingest pattern: the CLI emits a prompt file, the IDE's AI panel produces the response, the CLI ingests it. **Every LLM turn happens inside the AI session you already pay for — no API keys, no outbound network calls.**
5. **Halt for plan review.** Stage 2 stops the pipeline so you can approve, edit, merge, or drop domains before any source code is shipped to Stage 3.
6. **Output.** One `SKILL.md` per business feature lands in `<target-repo>/.github/skills/<feature-id>/`. Commit those files and from then on Copilot/Claude start every conversation already knowing the feature — fewer premium-request iterations, more first-time-correct answers.
