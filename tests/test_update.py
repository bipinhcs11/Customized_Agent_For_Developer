...
All tests use stdlib only (tempfile, pathlib, datetime, re, unittest).
"""
...
class TestBumpVersion(unittest.TestCase):
    ...
    def test_no_space_after_colon_matched(self):
        # \s* in the regex allows zero spaces
        md = "---\nversion:3\ndomain: bar\n---\n"
        old, new = _bump_version(md)
...
class TestDomainFromSkill(unittest.TestCase):
    ...
    def test_xml_sources_have_annotation(self):
...
def _setup_repo(tmpdir: str, feature_id: str, version: int,
    ...
    """Create a fake repo tree; return (repo_root, responses_dir)."""
    repo = Path(tmpdir)
    # Skills dir under .github/skills (first candidate in _resolve_skills_dir)
...
class TestIngestResponses(unittest.TestCase):
    def test_version_bumped_in_written_skill(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # existing skill is version 3; response says version 4 (AI honoured the +1)
            ...
            # The version bump logic forces new_ver = old_ver + 1 = 4, regardless
            # of what the AI wrote; since AI said 4 and old was 3, result is 4.
    ...
    def test_ingest_forces_correct_version_when_ai_skipped_bump(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            # AI forgot to bump; response has same version as existing
            response = _make_skill(feature, version=3)
            repo, rdir = _setup_repo(td, feature, version=3, response_content=response)
            result = ingest_responses(repo, responses_dir=rdir,
                                      commit=False, validate_schema=False)
            self.assertEqual(len(result.get("updated", [])), 1)
    ...
    def test_missing_response_file_reported_as_failed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Point responses_dir to a dir with no files for this feature
            rdir = repo / ".skill-gen" / ".update-responses"
            rdir.mkdir(parents=True)
            # Request only this one feature so ingest looks for the (absent) file
            result = ingest_responses(repo, responses_dir=rdir,
    ...
    def test_missing_skill_md_reported_as_failed(self):
        with tempfile.TemporaryDirectory() as td:
            feature = "no-skill"
            repo = Path(td)
            # Create skills dir but no SKILL.md for this feature
    ...
    def test_no_skills_dir_returns_early(self):
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            # No .github/skills or skills directory at all
